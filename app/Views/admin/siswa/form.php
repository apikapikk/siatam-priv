    ...
    <div class="bg-white rounded-2xl p-5 md:p-6 shadow-sm border border-gray-200/80">
        <form action="<?= $isEdit ? '/admin/siswa/' . $siswa['id'] . '/update' : '/admin/siswa/simpan' ?>" method="POST" class="flex flex-col gap-6">
            ...
            <div class="flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-1.5">
                    <label class="text-xs font-semibold text-gray-700">NIS <small class="font-normal text-gray-500">(Opsional)</small></label>
                    <div class="relative flex items-center">
                        <span class="material-symbols-outlined absolute left-3.5 text-gray-400 text-[18px]">tag</span>
                        <input type="text" name="nis" value="<?= e($siswa['nis'] ?? '') ?>" placeholder="Nomor induk siswa" class="w-full pl-10 pr-4 py-2.5 bg-[#FAF9F7] text-sm text-gray-800 rounded-xl border border-gray-200 focus:outline-none focus:border-[#324f47] focus:ring-1 focus:ring-[#324f47] transition-all">
                    </div>
                </div>
