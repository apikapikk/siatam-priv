<div class="content-section">
    ...
    <form action="/admin/pertemuan/<?= $pertemuan['id'] ?>/presensi/update" method="POST">
            ...
            <?php else: ?>
                <?php foreach ($presensiList as $item): ?>
                    <div class="ui-card" style="margin-bottom: 12px; padding: 14px;">
                        ...
                        <div>
                            <label class="form-label">Catatan Tentor</label>
                            <input type="text" name="presensi[<?= $item['siswa_id'] ?>][catatan]" value="<?= e($item['catatan'] ?? '') ?>" placeholder="Catatan perkembangan atau evaluasi siswa" class="form-control">
                        </div>
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>
