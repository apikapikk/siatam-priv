<div class="flex flex-col gap-5 w-full">
    ...
    <div class="flex items-center justify-between w-full">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl font-bold text-[#2D3E39] tracking-tight">Pusat Pertemuan & Presensi</h1>
                <p class="text-xs text-gray-500">Kelola sesi kelas aktual & verifikasi presensi siswa</p>
            </div>
        </div>
    ...
    <a href="/admin/pertemuan/tambah" class="w-full py-3 px-4 bg-[#324f47] hover:bg-[#2D3E39] active:scale-[0.99] text-white font-medium rounded-xl flex items-center justify-center gap-2 shadow-md transition-all">
    ...
    <div class="relative flex items-center w-full bg-white rounded-xl shadow-sm border border-gray-200/80 overflow-hidden focus-within:border-[#2d5a4c] transition-colors">
        ...
        <input class="w-full py-3 pr-4 bg-transparent text-sm text-gray-800 placeholder:text-gray-400 focus:outline-none" id="search-pertemuan" placeholder="Cari nama tentor, kelas, atau tanggal..." type="text"/>
    </div>

    <!-- List Data Pertemuan -->
    <div class="flex flex-col gap-3.5 w-full" id="pertemuan-card-list">
        <?php if (empty($pertemuanList)): ?>
            <div class="bg-white rounded-2xl p-8 text-center text-gray-500 border border-gray-200/80 text-sm">
                Belum ada data sesi pertemuan mengajar.
            </div>
        <?php else: ?>
            <?php foreach ($pertemuanList as $item): ?>
                <div class="pertemuan-card bg-white rounded-2xl p-4.5 shadow-sm border border-gray-200/80 flex flex-col gap-3 transition-all hover:shadow-md">
                    ...
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-3 bg-[#FAF9F6] p-3 rounded-xl border border-gray-100">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex flex-col">
                                <span class="text-xs font-bold text-gray-900"><?= e($item['tanggal']) ?> (<?= substr($item['jam_mulai'], 0, 5) ?> - <?= substr($item['jam_selesai'], 0, 5) ?> WIB)</span>
                                <span class="text-xs text-gray-500 mt-0.5">Tentor Pengajar: <strong class="text-gray-800"><?= e($item['tentor_nama']) ?></strong></span>
                            </div>
                        </div>
                    ...
                    <div class="flex items-center justify-end gap-2 pt-1 border-t border-gray-100">
                        <a href="/admin/pertemuan/<?= $item['id'] ?>/presensi" class="px-3.5 py-1.5 rounded-xl bg-[#324f47] text-white hover:bg-[#2D3E39] text-xs font-semibold transition-colors flex items-center gap-1 shadow-xs">
                            <span class="material-symbols-outlined text-[16px]">fact_check</span>
                            Kelola Presensi
                        </a>
                        <form action="/admin/pertemuan/<?= $item['id'] ?>/hapus" method="POST" onsubmit="return confirm('Yakin ingin menghapus data pertemuan ini?');" class="m-0">
...
<script>
    document.getElementById('search-pertemuan')?.addEventListener('input', function(e) {
        const query = e.target.value.toLowerCase().trim();
        const cards = document.querySelectorAll('#pertemuan-card-list .pertemuan-card');
        cards.forEach(card => {
